Feature: User Permissions
  ...
  Background:
    ...
    And article created by UNIQ_USER2[:email] user with parameters:
  ...
  Scenario: user can open User page
    When I logged as UNIQ_USER1[:email] user
    And I click on UNIQ_USER2[:email] user on Users list page
  ...
  Scenario: admin user can open Home page
    When I logged as UNIQ_USER2[:email] user
    Then I should be redirected to Home page
